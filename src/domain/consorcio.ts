...
export function calcularConsorcio(p: ConsorcioParams): ResultadoSimulacao {
  ...
  // CET ajustado: crédito trazido a valor presente descontado pelo IPCA anual
  // (expoente k/12 para converter parcelas em anos), depois composto com a
  // valorização do bem informada: (1 + CET_PV) × (1 + valorização) − 1.
  ...
  const tirMensalPV = irr(fluxoPV)
  const cetPV = Math.pow(1 + tirMensalPV, 12) - 1
